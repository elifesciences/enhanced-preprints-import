...
##
## Optionally, you can also previde a non-default workflow ID prefix as the second parameter. The docmap has will be appended to this string
...
temporal workflow start -n epp--prod --type importDocmap  -i '"'$docmap_url'"' -t 'epp' -w "$workflow_id_prefix$docmap_id_hash" --id-reuse-policy=TerminateIfRunning
