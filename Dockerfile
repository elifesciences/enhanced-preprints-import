FROM --platform=$BUILDPLATFORM node:20@sha256:5e19c1e2a2d7d0aafa58ed8927045f71c8fcf87435c2117dc4842c9393855453 as deps
RUN mkdir /app
WORKDIR /app
...
FROM --platform=$TARGETPLATFORM node:20@sha256:5e19c1e2a2d7d0aafa58ed8927045f71c8fcf87435c2117dc4842c9393855453 as platform_deps
RUN mkdir /app
WORKDIR /app
RUN apt-get update && apt-get install -y git python3 build-essential libc-dev
...
FROM node:20@sha256:5e19c1e2a2d7d0aafa58ed8927045f71c8fcf87435c2117dc4842c9393855453 as base
RUN mkdir /app
WORKDIR /app
